...
export default function Users() {
  ...
  const loadUsers = useCallback(async (manualRefresh = false) => {
    try {
      if (manualRefresh) {
  ...
  const portalConfiguredCount = users.filter(
    (u) =>
      u?.portalCredentialsConfigured === true ||
      u?.portal_credentials_configured === true
  ).length;
  ...
  const executeBackgroundSyncCheck = useCallback(
    async (studentId, startTime) => {
      if (!studentId) return;
      ...
      try {
        ...
        if (
          isSyncCompleted(currentStudent) &&
          !isSyncRunning(currentStudent)
        ) {
          setIsSyncing(false);
  ...
  const startBackgroundSyncFlow = async (studentId) => {
    if (!studentId) {
    ...
    try {
      setSyncMessage(
        "Starting AMS and Parent Portal synchronization in the background..."
      );
  ...
  const handleCreateStudent = async (event) => {
    ...
    const hasPortalUsername =
      Boolean(portalUsername);

    const hasPortalPassword =
      Boolean(portalPassword);
